...
window.onload = function () {
  ...
  if (gSettings.widgets.length > 0) {
    ...
    for (let n1 = 0; n1 < gSettings.widgets.length; n1++) {
      ...
      if (gSettings.widgets[n1].type === "glances") {
        ...
        setInterval(
          refreshWidgetGlances,
          gSettings.widgets[n1].settings.refreshMs,
          n1
        );
  ...
  for (let n1 = 0; n1 < gSettings.sections.length; n1++) {
    ...
    for (let n2 = 0; n2 < gSettings.sections[n1].tiles.length; n2++) {
      ...
      let thisDot = document.getElementById(
        "dot" +
          gSettings.sections[n1].name +
          gSettings.sections[n1].tiles[n2].name
      );
  ...
  for (let n1 = 0; n1 < gSettings.sections.length; n1++) {
    ...
    for (let n2 = 0; n2 < gSettings.sections[n1].tiles.length; n2++) {
      ...
      if (gSettings.sections[n1].tiles[n2].info) {
        ...
        tileLink.setAttribute(
          "data-bs-title",
          gSettings.sections[n1].tiles[n2].info
        );
      }
...
function refreshWidgetGlances(nW) {
  $.getJSON({
    url: gSettings.widgets[nW].settings.url + "api/3/quicklook",
  }).done(function (result, status, xhr) {
    document.getElementById("cpuPrct" + nW).innerText = result.cpu + "%";
    document.getElementById("memPrct" + nW).innerText = result.mem + "%";
  });

  $.getJSON({ url: gSettings.widgets[nW].settings.url + "api/3/sensors" }).done(
    function (result, status, xhr) {
      document.getElementById("cpuTemp" + nW).innerText =
        result.length > 0 ? result[0].value + "C" : "-";
    }
  );
}
